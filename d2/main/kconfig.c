...
/*
 * Rows are keyboard, joystick, mouse; the index into each is the item's id in
 * the matching kc_* table below.
 *
 * The joystick row was written for a 1990s flightstick - one stick for pitch
 * and turn, with slide, bank and throttle unbound - which leaves a pad with a
 * dead left stick and no way to move. It is now a twin-stick layout, taken from
 * a configuration set by hand on a Retroid Pocket Flip 2 rather than guessed:
 *
 *   right stick  aim (pitch, turn)      left stick  move (slide L/R, thrust)
 *   RT  fire primary                    RB  fire secondary
 *   LB  slide up                        LT  slide down
 *   A   bank right                      X   bank left
 *   Y   cycle primary                   B   cycle secondary
 *   d-pad right  flare                  d-pad down  rear view
 *   Back  automap
 *
 * Bank has no axis - there is no third stick - and stays on the two buttons.
 */
...
{0x19,0x5,0xff,0xff,0xff,0xff,0xff,0xff,0x4,0x13,0xff,0x2,0x0,0x4,0x0,0x3,0x0,0x0,0x0,0xff,0x0,0xff,0x0,0x1,0x0,0xd,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x6,0xff,0xff,0xff,0xff,0xff,0x0,0x0,0x0,0x0},
